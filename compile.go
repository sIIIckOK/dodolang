...
func compileProgram(strTokens []StringToken, tokens []Token) {
    ...
    assert(TokenCount == 27, "Exhaustive switch case for CompileProgram")
    for i := 0; i < len(tokens); i++ {
        ...
        case TokenVar:
            i++; varNameToken := tokens[i]
            i++; varSizeToken := tokens[i]
            if varSizeToken.Type != TokenInt {
                fmt.Printf("%v:%v:%v Expected int found `%v`", 
                    varSizeToken.Loc.FilePath, varSizeToken.Loc.Line, varSizeToken.Loc.Col, varSizeToken.Type)
            }
